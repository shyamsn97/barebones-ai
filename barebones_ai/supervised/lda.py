...
class LDA:
    ...
    def generate_priors(self):
        return np.log(
            (np.unique(self.y, return_counts=True)[1] / self.y.shape[0]).reshape(
                1, np.unique(self.y).shape[0]
            )
        )
    ...
    def generate_mu(self):
        ...
        if X.shape[1] == 1:
            ...
            self.params["mu"] = np.array(self.params["mu"]).reshape(
                1, uniquevals.shape[0]
            )
        else:
            for i in uniquevals:
                find = np.where(y == i)[0]
    ...
    def multd(self, predictors):
        ...
        for i in np.unique(y):
            ...
            second = ones.dot(
                (mureshaped / 2).dot(np.linalg.inv(sigma)).dot(mureshaped.T)
            )
